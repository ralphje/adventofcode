class Mapping:
    """Provides information on a mapping. Primarily, the set of ranges the mapping applies to."""

    def __init__(self) -> None:
        self.ranges: set[tuple[range, int]] = set()

    def add_range(self, destination: int, source: int, size: int) -> None:
        """Add a range with the given parameters to the internal store."""
        self.ranges.add((range(source, source + size), destination - source))

    def convert(self, value: int) -> int:
        """Convert the provided value to the correct value."""
        for source_range, difference in self.ranges:
...
class Mappings(list[Mapping]):
    """Set of multiple mappings, with a convert method that allows adding multiple mappings
    consecutively.
    """

    def new(self) -> Mapping:
        """Add new mapping to the list, and return the newly added mapping."""
        self.append(mapping := Mapping())
        return mapping

    def convert(self, value: int) -> int:
        """Use consecutive mappings to convert a value."""
        for mapping in self:
...
def part_1(document: str) -> int:
    ...
    * Seed 13, soil 13, fertilizer 52, water 41, light 34, temperature 34, humidity 35, location 35.
    ...
    return min(mappings.convert(seed) for seed in map(int, sections[0].split(": ")[1].split()))
